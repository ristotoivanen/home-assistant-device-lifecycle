...
This document defines the Asset Core and Asset Exposure invariants through Device Lifecycle 0.7.6. Future releases must extend the model through explicit migrations instead of replacing Asset identity.
...
Device Lifecycle 0.7.6 uses Store major version `3`, minor version `1`, and parent ConfigEntry version `4`. Asset Exposure remains derived and adds no stored projection IDs, exposure state, workflow drafts, or alternate Asset identity. Versions 0.7.1 through 0.7.6 require no Store or ConfigEntry migration.
